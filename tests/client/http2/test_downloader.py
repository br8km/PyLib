...
from pylib.client.http2.downloader import HTTPDownloader

from .test_abs import AbsTester


class TesterDownloader(AbsTester):
    ...
    def test_download_zip(self, tmp_path: Path) -> None:
        ...
        # new http downloader client with random proxy|ua and longer timeout
        http = HTTPDownloader(
            user_agent=self.user_agent,
            proxy_url=self.proxy_socks,
            timeout=3600,
