...
# Cons
- Not Support `chunk_size` when streaming, Please use http.downloader instead.
...
class HTTPDownloader(HTTPBasic):
    ...
    def download_direct(
        ...
        response = self.get(url=file_url, stream=True, debug=debug)
        if response is None:
    ...
    def _resume_download(
        ...
        return self.get(url=file_url, stream=True, debug=debug)
